...
fn describe_unsupported_dx12_command(commands: &[FrameCommand]) -> Option<&'static str> {
    commands.iter().find_map(|command| match command {
        ...
        FrameCommand::Line { .. } => None,
        FrameCommand::Circle { .. } => None,
        FrameCommand::Polyline { .. } => None,
...
impl ApplicationHandler<WindowsUserEvent> for WindowsApp {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        if matches!(requested_backend, DesktopRenderBackendKind::D3d12) {
            ...
            match hwnd
                .map(|hwnd| Dx12Backend::try_bind_hwnd(hwnd, size.width as i32, size.height as i32))
            {
                Some(Ok(backend)) => {
                ...
                Some(Err(err)) => {
                    ...
                    update_backend_detail(
                        &mut state,
                        format!(
                            "Windows D3D12 backend unavailable: {err}; falling back to software"
                        ),
                    );
                }
                None => {
...
fn present(
    ...
    if width == 0 || height == 0 {
        ...
        update_backend_detail(
            &mut state,
            "Windows surface is zero-sized; skipping present until the window is drawable",
        );
        return;
    }
...
fn scale_frame_command(command: FrameCommand, scale: f32) -> FrameCommand {
    match command {
        ...
        FrameCommand::Circle {
            ...
            radius: (radius.max(1.0) * scale).max(1.0),
            color,
        },
        FrameCommand::Polyline {
            points,
...
fn rasterize_text_command(request: &TextRequest) -> Option<(UiRect, DecodedImage)> {
    ...
    // Dropped for the same reason as `linux.rs`, which is where this bug
    // was actually caught: the caller's clip is in screen space and this
    // buffer is a private texture with its own origin, so intersecting
...
mod tests {
    ...
    fn a_multi_line_block_away_from_the_origin_keeps_its_first_lines() {
        // The twin of `linux.rs`'s test of the same name. That backend had
        // this bug for real — a text block's screen-space clip reached the
        // rasterizer alongside a texture-local rect, so a block at (40, 80)
        // lost its first lines and its left edge. This one shares the code
        // shape, so it gets the same guard rather than waiting for someone
        // to hit it on Windows.
        let mut request = sample_text_request();
        request.rect = UiRect {
            x: 40.0,
            y: 80.0,
