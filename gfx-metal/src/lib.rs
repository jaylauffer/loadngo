...
impl MetalBackend {
    ...
    pub fn try_bind_system_default() -> Result<Self, RendererError> {
        Err(RendererError::Backend(
            "Metal backend is only available on Apple platforms".to_string(),
        ))
    }
    ...
    pub fn try_bind_host_surface(&mut self) -> Result<(), RendererError> {
        Err(RendererError::Backend(
            "Metal surfaces are only available on Apple platforms".to_string(),
        ))
    }
    ...
    #[cfg(any(target_os = "macos", target_os = "ios"))]
    pub fn surface_logical_size_points(&self) -> Option<(f32, f32)> {
        self.surface
            .as_ref()
            ...
            match command {
                ...
                FrameCommand::Line {
                    ...
                    if let Some(geometry) = solid_geometry_from_points(
                        line_triangle_points(*from, *to, *thickness),
                        *color,
                    ) {
                ...
                FrameCommand::Circle {
                    ...
                    if let Some(geometry) =
                        solid_geometry_from_points(circle_triangle_points(*center, *radius), *color)
                    {
                        visuals.push(FrameVisual::SolidGeometry(geometry));
                    }
                }
...
fn append_polyline_visuals(
    ...
    if let Some(geometry) =
        solid_geometry_from_points(polyline_triangle_points(points, thickness, closed), color)
    {
        visuals.push(FrameVisual::SolidGeometry(geometry));
    }
}

fn solid_geometry_from_points(points: Vec<Point>, color: Color) -> Option<SolidGeometry> {
    if points.is_empty() {
        return None;
    }
    Some(SolidGeometry {
...
fn polyline_triangle_points(points: &[Point], thickness: i32, closed: bool) -> Vec<Point> {
    ...
    let mut append_segment = |from: Point, to: Point| {
        ...
        vertices.extend_from_slice(&[p0, p1, p2, p2, p1, p3]);
    };
...
fn line_triangle_points(from: Point, to: Point, thickness: i32) -> Vec<Point> {
    polyline_triangle_points(&[from, to], thickness, false)
}

fn circle_triangle_points(center: Point, radius: f32) -> Vec<Point> {
...
fn arc_triangle_points(
    ...
    let points = arc_polyline_points(center, radius, start_angle, sweep_angle);
    polyline_triangle_points(&points, thickness, false)
}
...
fn rasterize_circle(
    ...
    let width = (radius * 2.0 + 2.0).max(1.0) as u32;
    let height = (radius * 2.0 + 2.0).max(1.0) as u32;
    let mut rgba = vec![0u8; width as usize * height as usize * 4];
    let cx = radius;
    let cy = radius;
    let r = radius;
...
mod macos {
    ...
    use crate::{
        BlitImage, ClearColor, FontLineMetrics, Point, RasterMetrics, RasterizedText, SolidRect,
    };
    ...
    impl MetalSurface {
        ...
        pub fn bind_to_view(
            ...
            #[cfg(target_os = "ios")]
            {
                let scale: f64 = unsafe { msg_send![&*view, contentScaleFactor] };
                let host_layer: *mut AnyObject = unsafe { msg_send![&*view, layer] };
                if host_layer.is_null() {
                    return Err(RendererError::Backend(
                        "Host UIKit view has no backing layer".to_string(),
                    ));
                }
                unsafe {
                    let _: () = msg_send![&*layer, setContentsScale: scale];
                    let _: () = msg_send![host_layer, addSublayer: &*layer];
                }
            }
        ...
        pub fn logical_size_points(&self) -> (f32, f32) {
    ...
    pub fn present_scene_ordered(
        ...
        let (surface_width, surface_height) = surface.logical_size_points();
        ...
        for visual in visuals {
            match visual {
                ...
                PreparedVisual::SolidGeometry { points, color } => {
                    if !matches!(active_pipeline, Some(ActivePipeline::Solid)) {
                        let pipeline = solid_pipeline.ok_or_else(|| {
                            RendererError::Backend(
                                "Metal solid pipeline was unavailable for geometry rendering"
                                    .to_string(),
                            )
                        })?;
                        unsafe {
                    ...
                    let vertices = geometry_vertices(points, surface_width, surface_height);
                    unsafe {
                        let _: () = msg_send![
                            encoder,
                            setVertexBytes: vertices.as_ptr().cast::<c_void>(),
    ...
    fn geometry_vertices(
        ...
        points
            .iter()
            .map(|point| MetalVertex {
                x: clip_x(point.x, surface_width),
                y: clip_y(point.y, surface_height),
            })
            .collect()
    }
                ...
                if frame_index <= 8 || frame_index % 60 == 0 {
                    ...
                    for visual in &visuals {
                        match visual {
                            ...
                            FrameVisual::SolidGeometry(_) => solid_geometry += 1,
                    ...
                    metal_diagnostics_log(format!(
                        "frame={} commands={} text_commands={} visuals={} solid={} solid_geometry={} registered={} generated={} generated_text={} generated_shape={} generated_bytes={} transient_uploads={} transient_upload_bytes={} registered_reuses={} registered_texture_pool={} cache_entries={} cache_bytes={} cache_hits={} cache_misses={}",
                        frame_index,
            ...
            if visuals.is_empty() {
                ...
                let prepared_visuals = visuals
                    ...
                    .map(|visual| match visual {
                        ...
                        FrameVisual::SolidGeometry(geometry) => {
                            Ok(macos::PreparedVisual::SolidGeometry {
...
mod tests {
    ...
    #[test]
    fn line_visuals_use_solid_geometry() {
        let mut backend = MetalBackend::new_headless();
        backend.recorded_commands = vec![FrameCommand::Line {
            from: ui_core::geometry::Point { x: 0.0, y: 0.0 },
            to: ui_core::geometry::Point { x: 16.0, y: 8.0 },
            color: Color::rgba(255, 0, 0, 255),
            thickness: 2,
        }];
        let visuals = backend.frame_visuals().expect("line visuals should build");
        assert!(matches!(
            visuals.as_slice(),
            [FrameVisual::SolidGeometry(_)]
        ));
    }

    #[test]
    fn circle_visuals_use_solid_geometry() {
        let mut backend = MetalBackend::new_headless();
        backend.recorded_commands = vec![FrameCommand::Circle {
            center: ui_core::geometry::Point { x: 8.0, y: 8.0 },
            radius: 4.0,
            color: Color::rgba(0, 255, 0, 255),
        }];
        let visuals = backend
            .frame_visuals()
            .expect("circle visuals should build");
        assert!(matches!(
            visuals.as_slice(),
            [FrameVisual::SolidGeometry(_)]
        ));
    ...
    #[test]
    fn polyline_visuals_use_solid_geometry() {
        let mut backend = MetalBackend::new_headless();
        backend.recorded_commands = vec![FrameCommand::Polyline {
            points: vec![
                ui_core::geometry::Point { x: 0.0, y: 0.0 },
                ui_core::geometry::Point { x: 16.0, y: 8.0 },
                ui_core::geometry::Point { x: 24.0, y: 4.0 },
            ],
        ...
        let visuals = backend
            .frame_visuals()
            .expect("polyline visuals should build");
        assert!(matches!(
            visuals.as_slice(),
            [FrameVisual::SolidGeometry(_)]
        ));
    ...
    #[test]
    fn arc_visuals_use_solid_geometry() {
        let mut backend = MetalBackend::new_headless();
        backend.recorded_commands = vec![FrameCommand::Arc {
            center: ui_core::geometry::Point { x: 16.0, y: 16.0 },
            radius: 10.0,
            start_angle: 0.0,
        ...
        let visuals = backend.frame_visuals().expect("arc visuals should build");
        assert!(matches!(
            visuals.as_slice(),
            [FrameVisual::SolidGeometry(_)]
        ));
    ...
    fn text_layout_objects_are_released_after_measure_and_raster() {
        let baseline_layouts = super::macos::live_text_layout_count();
        let mut observed_peak = baseline_layouts;
        ...
        for _ in 0..32 {
            ...
            observed_peak = observed_peak.max(super::macos::live_text_layout_count());
        }

        let final_layouts = super::macos::live_text_layout_count();
        assert!(
            final_layouts <= baseline_layouts + 1,
            "text layouts must return to baseline after repeated measure/raster cycles: baseline={baseline_layouts} final={final_layouts} peak={observed_peak}"
        );
    }
