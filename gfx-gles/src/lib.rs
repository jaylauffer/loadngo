...
mod android {
    ...
    pub fn present_scene(
        ...
        unsafe {
            ...
            // matters for draw order (switching pipelines, or `Clear`,
            // which does a real GL op the accumulated draws must precede).
            let mut solid_batch: Vec<f32> = Vec::new();
            let mut image_batch: Vec<f32> = Vec::new();
            let mut image_batch_texture: Option<GlTexture> = None;
    ...
    fn ensure_solid_pipeline(
        ...
        unsafe {
            if *solid_program == 0 {
                // Color is a per-vertex attribute, not a uniform: batching
                // many differently-colored shapes into one draw call (see
    ...
    fn ensure_textured_pipeline(
        ...
        unsafe {
            if *textured_program == 0 {
                // Tint is a per-vertex attribute, not a uniform, for the
                // same reason as the solid pipeline's color: it must be able
                // to vary within one batched draw call.
                let vertex_shader = compile_shader(
                    GL_VERTEX_SHADER,
    ...
    // ------------------------------------------------------------------
    //
    // `present_scene` no longer issues one `glBufferData`+`glDrawArrays`
    // pair per shape/sprite. Every solid shape or sprite in a command run
    // is instead appended (CPU-side, no GL calls) to a shared accumulator
    // by the `push_*` functions below, and the whole run is submitted as
    // one buffer upload and one draw call by `flush_solid_batch` /
    // `flush_image_batch` -- only when the next command needs a different
    // pipeline or texture, or the command stream ends. A scene with many
    // enemies/projectiles/effects on screen used to cost one GPU submission
    // per entity; it now costs one per contiguous run of same-pipeline,
    // same-texture draws, which is what let color/tint move from a uniform
    // to a per-vertex attribute above -- a uniform can't vary within a
    // single batched draw.
    ...
    /// Appends `positions` (flat x,y pairs) to `batch`, interleaving in
    /// `color` per vertex.
    ...
    /// Submits every vertex accumulated in `batch`, sampling `texture`, as
    /// one draw call, then clears it. A no-op when nothing was accumulated.
    fn flush_image_batch(
        ...
        unsafe {
            ...
            if u_tex < 0 {
                return Err(RendererError::Backend(
                    "u_tex uniform not found in GLES textured program".to_string(),
                ));
            }
