...
impl GraphicsBackend for GlesBackend {
    ...
    fn end_frame(&mut self) -> Result<(), RendererError> {
        ...
        if !self.supports_commands(&self.recorded_commands) {
            return Err(RendererError::Backend(
                "Android GLES backend only supports clear-only frames right now".to_string(),
            ));
        }
...
mod tests {
    ...
    #[test]
    fn collect_solid_rects_expands_stroke_rects() {
        let rects = collect_solid_rects(&[FrameCommand::StrokeRect {
            rect: ui_core::geometry::Rect {
                x: 2,
                y: 4,
                width: 20,
