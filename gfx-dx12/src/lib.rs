...
mod windows_backend {
        ...
        pub fn try_bind_hwnd(hwnd: isize, width: i32, height: i32) -> Result<Self, RendererError> {
            unsafe {
                ...
                command_list.Close().map_err(|err| {
                    RendererError::Backend(format!("initial command list close failed: {err}"))
                })?;
                ...
                let swap_chain: IDXGISwapChain3 = factory
                    ...
                    .map_err(|err| {
                        RendererError::Backend(format!("CreateSwapChainForHwnd failed: {err}"))
                    })?;
                ...
                let rtv_heap = device
                    ...
                    .map_err(|err| {
                        RendererError::Backend(format!("CreateDescriptorHeap RTV failed: {err}"))
                    })?;
                let srv_heap = device
                    .CreateDescriptorHeap(&D3D12_DESCRIPTOR_HEAP_DESC {
                    ...
                    .map_err(|err| {
                        RendererError::Backend(format!("CreateDescriptorHeap SRV failed: {err}"))
                    })?;
                let fence = device
                    .CreateFence(0, D3D12_FENCE_FLAG_NONE)
                    .map_err(|err| RendererError::Backend(format!("CreateFence failed: {err}")))?;
                let fence_event = CreateEventW(None, false, false, None)
                    .map_err(|err| RendererError::Backend(format!("CreateEventW failed: {err}")))?;
    ...
    impl Dx12Backend {
                ...
                for index in 0..FRAME_COUNT {
                    let resource: ID3D12Resource =
                        self.swap_chain.GetBuffer(index as u32).map_err(|err| {
                            RendererError::Backend(format!("swapchain GetBuffer failed: {err}"))
                        })?;
        ...
        fn resize_if_needed(&mut self) -> Result<(), RendererError> {
            unsafe {
                ...
                let desc = self
                    .swap_chain
                    .GetDesc1()
                    .map_err(|err| RendererError::Backend(format!("GetDesc1 failed: {err}")))?;
                ...
                self.swap_chain
                    ...
                    .map_err(|err| {
                        RendererError::Backend(format!(
                            "ResizeBuffers failed from {}x{} to {}x{}: {err}",
                            desc.Width, desc.Height, self.surface_width, self.surface_height
                        ))
                    })?;
        ...
        fn wait_for_gpu(&mut self) -> Result<(), RendererError> {
            unsafe {
                ...
                self.command_queue
                    ...
                    .map_err(|err| RendererError::Backend(format!("queue Signal failed: {err}")))?;
                ...
                if self.fence.GetCompletedValue() < value {
                    self.fence
                        ...
                        .map_err(|err| {
                            RendererError::Backend(format!("SetEventOnCompletion failed: {err}"))
                        })?;
        ...
        fn ensure_vertex_buffer(&mut self, required_size: usize) -> Result<(), RendererError> {
            ...
            unsafe {
                ...
                self.device
                    ...
                    .map_err(|err| {
                        RendererError::Backend(format!("vertex buffer allocation failed: {err}"))
                    })?;
                let resource = resource.ok_or_else(|| {
                    RendererError::Backend(
                        "vertex buffer allocation returned no resource".to_string(),
                    )
                })?;
        ...
        unsafe fn upload_texture(
            ...
            upload
                ...
                .map_err(|err| RendererError::Backend(format!("upload map failed: {err}")))?;
            ...
            for command in &commands {
                match command {
                    ...
                    FrameCommand::Line {
                        ...
                        let vertex_count =
                            line_triangle_points(*from, *to, *thickness).len() as u32;
                        if vertex_count > 0 {
                            draws.push(DrawItem {
                    ...
                    FrameCommand::Circle {
                        ...
                        let points = circle_triangle_points(*center, *radius);
                    ...
                    FrameCommand::Polyline {
                        ...
                        let triangle_points =
                            polyline_triangle_points(points.as_slice(), *thickness, *closed);
                        let vertex_count = triangle_points.len() as u32;
                        append_solid_vertices(
                            &mut vertices,
                            triangle_points,
                            *color,
                    ...
                    FrameCommand::Arc {
                        ...
                        let triangle_points = arc_triangle_points(
                            *center,
                            *radius,
                            *start_angle,
                            *sweep_angle,
        ...
        fn draw_frame(&mut self) -> Result<(), RendererError> {
            unsafe {
                ...
                let current = self
                    ...
                    .cloned()
                    .ok_or_else(|| RendererError::Backend("back buffer unavailable".to_string()))?;
                self.command_list.ResourceBarrier(&[transition_barrier(
                    &current,
                ...
                self.swap_chain
                    ...
                    .ok()
                    .map_err(|err| RendererError::Backend(format!("Present failed: {err}")))?;
                self.wait_for_gpu()
            }
        }
    }
    ...
    fn create_root_signature(device: &ID3D12Device) -> Result<ID3D12RootSignature, RendererError> {
        unsafe {
            ...
            .map_err(|err| {
                RendererError::Backend(format!("D3D12SerializeRootSignature failed: {err}"))
            })?;
            let blob = blob
                .ok_or_else(|| RendererError::Backend("root signature blob missing".to_string()))?;
            device
                .CreateRootSignature(
                    0,
                ...
                .map_err(|err| RendererError::Backend(format!("CreateRootSignature failed: {err}")))
        }
    }
    ...
    fn create_pipeline_state(
        ...
        unsafe {
            ...
            device.CreateGraphicsPipelineState(&desc).map_err(|err| {
                RendererError::Backend(format!("CreateGraphicsPipelineState failed: {err}"))
            })
        }
    }
    ...
    fn compile_shader(source: &str, entry: &str, target: &str) -> Result<ID3DBlob, RendererError> {
        unsafe {
            let entry =
                CString::new(entry).map_err(|err| RendererError::Backend(err.to_string()))?;
            let target =
                CString::new(target).map_err(|err| RendererError::Backend(err.to_string()))?;
            let source =
                CString::new(source).map_err(|err| RendererError::Backend(err.to_string()))?;
            let mut blob = None;
            let mut error_blob = None;
            D3DCompile(
                source.as_ptr() as _,
    ...
    fn append_solid_vertices(
        ...
        for point in points {
            let x = (point.x / surface_width.max(1) as f32) * 2.0 - 1.0;
            let y = 1.0 - (point.y / surface_height.max(1) as f32) * 2.0;
    ...
    fn line_triangle_points(from: Point, to: Point, thickness: i32) -> Vec<Point> {
        ...
        vec![p0, p2, p3, p0, p1, p2]
    }

    fn circle_triangle_points(center: Point, radius: f32) -> Vec<Point> {
        let radius = radius.max(1.0);
        let segments = ((radius * 0.8).ceil() as usize).clamp(12, 64);
        ...
        for index in 0..segments {
            ...
            points.push(center);
            points.push(Point {
                x: center.x + theta0.cos() * radius,
                y: center.y + theta0.sin() * radius,
            });
            points.push(Point {
                x: center.x + theta1.cos() * radius,
                y: center.y + theta1.sin() * radius,
            });
        }
        points
    }
    ...
    fn polyline_triangle_points(points: &[Point], thickness: i32, closed: bool) -> Vec<Point> {
        ...
        for segment in points.windows(2) {
            triangles.extend(line_triangle_points(segment[0], segment[1], thickness));
        }
        if closed {
            triangles.extend(line_triangle_points(
                *points.last().unwrap(),
                points[0],
                thickness,
            ));
    ...
    fn arc_polyline_points(
        ...
        let segments = ((radius * sweep / 18.0).ceil() as usize).clamp(8, 96);
